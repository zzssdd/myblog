<template>
  <div>
    ...
    <div class="container">
      ...
      <hr>
      <div class="frontmatter-markdown post font-size:300%" v-html="post.content">
      </div>
      <Board :flag=1 :id=postId />
      <PostNavBtn :prev="prevPost" :next="nextPost" :flag=1 />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getPost() {
      ...
      this.postId = this.$route.path.split('article/')[1]
      const { data: res } = await this.$http.get(`article/${this.postId}` )
      this.post=res.data

      const curPostIdx = parseInt(this.postId)
        this.changeTitle(this.post.title)
        try {
          const { data: pre } = await this.$http.get(`article/${curPostIdx+1}` )
          this.prevPost = pre.data
          const { data: next } = await this.$http.get(`article/${curPostIdx-1}` )
